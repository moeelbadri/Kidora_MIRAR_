...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['add_figure'])) {
    ...
    if ($name && $desc) {
        $pdo->prepare("INSERT INTO history_figures (name,title,description,youtube_id,story_line) VALUES (?,?,?,?,?)")
            ->execute([$name,$title,$desc,$youtube ?: null,$story]);
...
<p style="color:var(--ink-soft);max-width:640px;">تظهر عشوائياً للطفل بعد إنجاز كل مهمة يومية. أضف رابط يوتيوب توعوي قصير (أقل من دقيقة، معرّف الفيديو فقط) إن توفّر.</p>
